...
class Content(object):
	headers = {
		...
		'Host':'music.163.com',
		'Cookie':'_iuqxldmzr_=32; _ntes_nnid=71af80ecaad581b9bb7e5a4c52be190f,1517311281571; _ntes_nuid=71af80ecaad581b9bb7e5a4c52be190f; __utmc=94650624; __utmz=94650624.1517311282.1.1.utmcsr=baidu|utmccn=(organic)|utmcmd=organic; __remember_me=true; playerid=53065803; __utma=94650624.1546687865.1517311282.1517536226.1517552701.10; JSESSIONID-WYYY=nMNZAiUpriachtfMBUQ8inqEqEaQgtvM0wAH%2FEwjV95k3Ba6%2BF%5CKhS%2BbVBMJi9qwduunM51pntOJBru2ONsoNXI6MEtxqyERAH3dARcyx%2Fg%2FqKSJwp5ac5YncNMq1juuJ6ErAU2uBIauj0%2BrW1DsgZTiM8nYa9bBVcaEBdMc3WVjuGy3%3A1517556636212; MUSIC_U=6ac9e69dfe236a25c057be873fa8b2474a1a81fad708b1a363d1aa5e705ffcc6ca25f8583fa778443e5f52793cd3151d3083d51696b770fcdf4b75875b604866747bc57857cce529bf122d59fa1ed6a2; __csrf=a2251ef0c8b8cd21461e8a7ec42bf4cd; __utmb=94650624.18.10.1517552701',		
...
#歌词
	...
	def save_lyric(self,music_id,flag):
		...
		flag =True
		if flag:
			r = requests.post('http://music.163.com/weapi/song/lyric?csrf_token=a2251ef0c8b8cd21461e8a7ec42bf4cd' + str(music_id),
		                      headers=self.headers, params=self.params, data=self.getData(music_id), proxies=self.proxies)
		else:
		    r = requests.post('http://music.163.com/weapi/song/lyric?csrf_token=' + str(music_id),
		                      headers=self.headers, params=self.params, data=self.data2, proxies=self.proxies1)
		# print(r.content)
		print(music_id)
		# print(r.json()["total"])
		return r.json()
...
if __name__ == '__main__':
	...
	def save_comments(musics, flag, connection0):
		for i in musics:
			try:
				...
				if content.has_key('lrc'):
					sql.insert_gfmusic(i["music_id"],content['lrc']['lyric'])
				else:
					sql.insert_gfmusic(i["music_id"],'null')
				# print(len(comments['comments']))
				# if comments["total"] > 0 :
				# 	count = 1
				# 	for j in comments['comments']:
				# 		# hotList.append(j["content"])
				# 		sql.insert_with_you(count,j['time'],j["content"], j['likedCount'],connection0)
				# 		count = count + 1
				...
				# print(str(e))
				time.sleep(5)
	music_before = sql.qry_un_gflist()
	...
	# save_comments([{'music_id':529802606}],True,connection1)
