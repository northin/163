...
def save_artist(group_id, initial):
    ...
    # writeFile(r"./arts.txt", r.content)
    all_arts = soup.find(id="m-artist-box").find_all("p");
    # all_artList = []
    for i in all_arts:  #s上面的
    	# all_artList.append(i.find("a")["href"].replace('/artist?id=', '').strip());
    	# all_artList.append(i.find("a").string);
    ...
    for i in artists: #下面的
    	# all_artList.append(i["href"].replace('/artist?id=', '').strip());
    	# all_artList.append(i.string);
...
gidList = [1001,1002,1003,2001,2002,2003,6001,6002,6003,7001,7002,7003,4001,4002,4003]
