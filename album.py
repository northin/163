...
class Album(object):
	...
	def save_album(self,artist_id):
		...
		all_albums = body.find_all('a', attrs={'class': 'msk'})

		# i["href"] = '/album?id=none'

		for i in all_albums: 
			id = i["href"].replace('/album?id=', '').strip()
			# name = i.string
			print(artist_id,id)

			sql.insert_album(id,artist_id)
if __name__ == '__main__':
	my_album = Album()
	artistList = sql.qry_artists()
	for i in artistList:
		try:
			my_album.save_album(i["href"])
			# print(i)
