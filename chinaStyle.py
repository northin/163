...
def save_playlist(page_id):
    params = {'order':'hot','cat':u'古风','limit':'35','offset':page_id}
    ...
    all_playList = soup.find(id="m-pl-container").find_all("li");
    for i in all_playList: 
        aa = i.find("a", attrs={'class': 'tit f-thide s-fc0'})
        id = aa["href"].replace('/playlist?id=', '').strip()
        name = aa.string
        zan = i.find("span", attrs={'class','nb'}).string
        print(id)
        sql.insert_playList(id,name,zan)


for gid in range(0,40):
    save_playlist(gid*35)
